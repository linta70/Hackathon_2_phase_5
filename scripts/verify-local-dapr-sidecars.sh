...
# Script to verify all services start with Dapr sidecars running in local Minikube environment
# This script should be run after deploying the application to Minikube
...
echo "Verifying services and Dapr sidecars in local Minikube namespace $NAMESPACE..."
...
echo "Checking Dapr components in local environment..."
dapr components -k --namespace $NAMESPACE

# Use Dapr CLI to check status of services
echo "Checking Dapr status in local environment..."
dapr status -k --namespace $NAMESPACE

# Check application logs for any Dapr-related issues
echo "Checking application logs for Dapr integration..."
for pod in $PODS; do
    echo "Logs for pod: $pod"
    kubectl logs $pod -n $NAMESPACE | grep -i dapr || echo "  No Dapr-related messages found in $pod"
done

if [ "$ALL_HEALTHY" = true ]; then
    echo "SUCCESS: All services are running with Dapr sidecars in local environment!"
    exit 0
else
    echo "WARNING: Some services may not be running optimally in local environment"
